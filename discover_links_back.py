"""
Discover who links back to eof.news by checking each DEVS_FEEDS site (derived from feed URL).
Output: list of { name, url } that contain a link to eof.news / ipsedigit.github.io.
Add those to LINKS_BACK in const.py and run --action devs to update the site.
"""
import re
import time
...
from const import DEVS_FEEDS

OUR_DOMAINS = ("eof.news", "ipsedigit.github.io")
USER_AGENT = "eof.news Backlink Discovery (https://ipsedigit.github.io/devs/)"
REQUEST_DELAY_SEC = 2
TIMEOUT = 15
...
def discover_links_back() -> list[dict]:
    found = []
    for i, feed_spec in enumerate(DEVS_FEEDS):
...
def main():
    print("Checking DEVS_FEEDS sites for pages that link to eof.news / ipsedigit.github.io ...")
    found = discover_links_back()
    print(f"\nFound {len(found)} site(s) that link to us.")
    if found:
        print("Add them to LINKS_BACK in const.py, then run: python main.py --action devs")
    return found
