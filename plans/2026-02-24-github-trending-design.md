# GitHub Trending Tracker — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a daily GitHub Trending page to eof.news that fetches popular repos via GitHub Search API, displays statistics with Chart.js graphs, and links to owner profiles.

**Architecture:** New `github_trending.py` module following the `models.py`/`cves.py` pattern: fetch from GitHub Search API, generate `docs/_data/github.json` and `docs/github/trending.md`. Chart.js loaded via CDN, data injected by Liquid from JSON.

**Tech Stack:** Python stdlib (urllib, json), GitHub Search API (no auth required), Chart.js CDN, Jekyll/Liquid templates.

---

### Task 1: Create `github_trending.py` — API fetch functions

**Files:**
- Create: `github_trending.py`

**Step 1: Create the module with constants and `_github_search()` helper**
...
def _github_search(query, sort="stars", per_page=30):
    """Search GitHub repos. Returns list of repo dicts. Handles auth and errors."""
    params = {
        "q": query,
        "sort": sort,
        "order": "desc",
...
```bash
git add github_trending.py
git commit -m "feat(github): add github_trending module with search helper"
```

---

### Task 2: Add `_fetch_trending_repos()` and `_parse_repo()`

**Files:**
- Modify: `github_trending.py`

**Step 1: Add `_parse_repo()` and `_fetch_trending_repos()` after `_github_search()`**
...
```bash
git add github_trending.py
git commit -m "feat(github): add repo fetching and parsing"
```

---

### Task 3: Add `_generate_page()` with Chart.js graphs

**Files:**
- Modify: `github_trending.py`

**Step 1: Add `_generate_page()` function**

This is the largest piece. The page includes:
- Summary badges (total repos, top language, most starred)
- Chart.js donut chart for language distribution
- Chart.js horizontal bar chart for top 10 repos by stars
- Repo cards with all metadata
...
```bash
git add github_trending.py
git commit -m "feat(github): add page generation with Chart.js graphs"
```

---

### Task 4: Add `publish_github_trending()` entry point
...
```bash
git add github_trending.py
git commit -m "feat(github): add publish_github_trending entry point"
```
...
**Files:**
- Modify: `main.py:4-21` (add case to match statement)
- Modify: `main.py:30` (update help text)

**Step 1: Add the `github` case**

In `main.py`, add after the `"models"` case (line 20):
...
**Step 2: Update the help text on line 30**
...
```bash
git add main.py
git commit -m "feat(github): wire up --action=github in main.py"
```
...
**Files:**
- Create: `.github/workflows/githubtrending.yml`

**Step 1: Create the workflow file** (copy pattern from `modeltracker.yml`)
...
jobs:
  run-github:
    ...
    - name: Run GitHub trending tracker
      run: python main.py --action=github
    ...
    - name: Commit and Push changes
      run: |
        ...
        git commit -m "Auto: GitHub trending update at $TIMESTAMP" || echo "No changes to commit"
...
```bash
git add .github/workflows/githubtrending.yml
git commit -m "ci: add daily GitHub trending tracker workflow"
```
...
**Step 1: Run the action**

```bash
PYTHONIOENCODING=utf-8 python main.py --action=github
```

Expected output:
```
Fetching trending GitHub repositories...
  GitHub Search: created:>YYYY-MM-DD stars:>10
    Rate limit remaining: XX
  GitHub Search: pushed:>YYYY-MM-DD stars:>1000
    Rate limit remaining: XX
  GitHub Search: created:>YYYY-MM-DD stars:>100
    Rate limit remaining: XX
  Total unique repos: ~30
Total trending repos: 30
  JSON written: docs/_data/github.json
  Page written: docs/github/trending.md
```
...
python -c "
import json
with open('docs/_data/github.json') as f:
    data = json.load(f)
print(f'Total repos: {data[\"total\"]}')
print(f'Top language: {data[\"top_language\"]}')
r = data['repos'][0]
print(f'First repo: {r[\"name\"]}')
for field in ['stars','forks','open_issues','language','license','last_push','owner_url','owner_avatar','repo_url','topics']:
...
Check that `docs/github/trending.md` exists and contains Chart.js script tags and Liquid template code.

**Step 4: Final commit**

```bash
git add docs/_data/github.json docs/github/trending.md
git commit -m "feat(github): initial GitHub trending data and page"
```
